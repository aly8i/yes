    ...
    <table class="w-full text-left z-20">
            ...
            {users.slice(startindex,finishindex+1).map((user)=>(
                  <tr onMouseEnter={()=>{setDetailview("userview");selectUser(user)}}>
                    ...
                    <td class="sm:p-3 py-2 px-1 border-b border-gray-800 md:table-cell hidden">{user?.box}</td>
                    <td class={`sm:p-3 py-2 px-1 border-b border-gray-800 ${user?.credit>0?`text-green-500`:`text-red-500`}`}> {user?.credit?formatMoney(user?.credit):"~"}</td>
                    <td class="sm:p-3 py-2 px-1 border-b border-gray-800">
                      <div class="flex items-center">
